...
class Cuti extends CI_Controller
{
    ...
    function formAddData()
    {
        if (($this->role == "admin") or ($this->role == "skpd")) {
            ...
            $this->load->view('keterangan_absensi/cuti_form_add', $data);
        } else {
            redirect(site_url('blocked'));
        }
    }
    ...
    function addData()
    {
        if (($this->role == "admin") or ($this->role == "skpd")) {
            ...
            if ($this->form_validation->run() == false) {
                ...
                $config['file_name']        = 'cuti-' . $idPegawai . '-' . date("Ymd-His");
                $config['max_size']         = 750;
                ...
                if (!$this->upload->do_upload('file_upload')) {
                    ...
                    $data = ['status' => FALSE,'tanggal' => TRUE,'pesan' => 'Tes'];
                    $this->output->set_content_type('application/json')->set_output(json_encode($data));
                    ...
                    while ($tglMulai <= $tglAkhir) {
                        ...
                        if (($day != 'Sat') and ($day != 'Sun')) {
                            ...
                            if ($cekHariLibur == 0) {
                                ...
                                if ($cekresultData >0) {
                                    ($resultData['id_keterangan_absensi'] == 0) ? $cekCuti = $cekCuti : $cekCuti++;
                                }
                    ...
                    $log = simpan_log("Insert Pegawai Pegawai Cuti", json_encode($string));
    ...
    function deleteData()
    {
        if (($this->role == "admin") or ($this->role == "skpd")) {
            ...
            $idCuti = decrypt_url($id);
            $temp = $this->master->selectDataId('keterangan_absensi', ['id_keterangan_absensi' => $idCuti]);
            $resultData = $this->master->selectDataBy('kehadiran', ['id_keterangan_absensi' => $idCuti]);
            ...
            $log = simpan_log("Delete Pegawai Cuti", json_encode($string));
            $res = $this->status->DeleteStatusAbsensi($idCuti, $dataKehadiranUpdate, $dataKehadiranDelete, $log);
            if ($res > 0) {
