...
class Sakit extends CI_Controller
{
    ...
    function formAddData()
    {
        if (($this->role == "admin") or ($this->role == "skpd")) {
            ...
            $this->load->view('keterangan_absensi/sakit_form_add', $data);
        } else {
            redirect(site_url('blocked'));
        }
    }
    ...
    function addData()
    {
        if (($this->role == "admin") or ($this->role == "skpd")) {
            ...
            if ($this->form_validation->run() == false) {
                ...
                $config['file_name']        = 'sakit-' . $idPegawai . '-' . date("Ymd-His");
                $config['max_size']         = 750;
                ...
                if (!$this->upload->do_upload('file_upload')) {
                    ...
                    while ($tglMulai <= $tglAkhir) {
                        ...
                        if (($day != 'Sat') and ($day != 'Sun')) {
                            ...
                            if ($cekHariLibur == 0) {
                                ...
                                if ($cekresultData >0) {
                                    ($resultData['id_keterangan_absensi'] == 0) ? $cekSakit = $cekSakit : $cekSakit++;
                                }
                    ...
                    $log = simpan_log("Insert Pegawai Pegawai Sakit", json_encode($string));
    ...
    function deleteData()
    {
        if (($this->role == "admin") or ($this->role == "skpd")) {
            ...
            $idSakit = decrypt_url($id);
            $temp = $this->master->selectDataId('keterangan_absensi', ['id_keterangan_absensi' => $idSakit]);
            $resultData = $this->master->selectDataBy('kehadiran', ['id_keterangan_absensi' => $idSakit]);
            ...
            $log = simpan_log("Delete Pegawai Sakit", json_encode($string));
            $res = $this->status->DeleteStatusAbsensi($idSakit, $dataKehadiranUpdate, $dataKehadiranDelete, $log);
            if ($res > 0) {
