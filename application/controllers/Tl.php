...
class Tl extends CI_Controller
{
    ...
    function loadData()
    {
        if ($this->role == "admin") {
            $data = [
                "menu" => "menu_izin",
                "submenu" => "submenu_tugas_luar",
                "result_skpd" => $this->master->selectDataBy($this->simpeg. '.skpd', ['is_active'=>'Y'], 'nama_skpd ASC'),
                "role" => $this->role
            ];
            $this->load->view('keterangan_absensi/tugas_luar_view', $data);
    ...
    function formAddData()
    {
        if (($this->role == "admin") or ($this->role == "skpd")) {
            ...
            $this->load->view('keterangan_absensi/tugas_luar_form_add', $data);
        } else {
            redirect(site_url('blocked'));
        }
    }
    ...
    function addData()
    {
        if (($this->role == "admin") or ($this->role == "skpd")) {
            ...
            if ($this->form_validation->run() == false) {
                ...
                $config['file_name']        = 'tl-' . $idPegawai . '-' . date("Ymd-His");
                $config['max_size']         = 750;
                ...
                if (!$this->upload->do_upload('file_upload')) {
                    ...
                    while ($tglMulai <= $tglAkhir) {
                        ...
                        if (($day != 'Sat') and ($day != 'Sun')) {
                            ...
                            if ($cekHariLibur == 0) {
                                ...
                                //($resultData['id_keterangan_absensi'] == 0) ? $cekTl = $cekTl : $cekTl++;
                    ...
                    $log = simpan_log("Insert Pegawai Pegawai Tugas Luar", json_encode($string));
                    ...
                    if (format_tanggal_database($tanggalMulai) < $tglBolehUpload) {
                        ...
                        $data = [
                            'status' => FALSE,
                            'tanggal' => TRUE,
                            'pesan' => 'Tanggalupload tidak diizinkan'
                        ];
                    } else {
    ...
    function deleteData()
    {
        if (($this->role == "admin") or ($this->role == "skpd")) {
            ...
            $idTl = decrypt_url($id);
            $temp = $this->master->selectDataId('keterangan_absensi', ['id_keterangan_absensi' => $idTl]);
            $resultData = $this->master->selectDataBy('kehadiran', ['id_keterangan_absensi' => $idTl]);
            ...
            $log = simpan_log("Delete Pegawai Tugas Luar", json_encode($string));
            $res = $this->status->DeleteStatusAbsensi($idTl, $dataKehadiranUpdate, $dataKehadiranDelete, $log);
            if ($res > 0) {
