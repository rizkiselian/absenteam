...
<div class="panel-header bg-primary-gradient">
    <div class="page-inner py-4">
        <div class="d-flex align-items-left align-items-md-center flex-column flex-md-row">
            <h3 class="text-white pb-3 fw-bold">DATA KEHADIRAN PEGAWAI <?= format_nama($pegawai['gelar_depan'], $pegawai['nama_pegawai'], $pegawai['gelar_belakang']); ?></h3>
        </div>
    </div>
</div>
            ...
            <div class="row">
                <div class="col-sm-12">
                    <fieldset class="scheduler-border">
                        <legend class="scheduler-border">FILTER KEHADIRAN</legend>
                        <div class="row p-2">
...
<script>
    $(document).ready(function() {
        $('.select2').select2();
        var pegawai = "<?= encrypt_url($pegawai['id_pegawai']); ?>";
        var bulan = $("#bulan").val();
        var tahun = $("#tahun").val();
        load_data(pegawai, bulan, tahun);
    });

    function load_data(pegawai, bulan, tahun) {
        $.ajax({
            url: '<?= site_url('load-detail-kehadiran-pegawai/'); ?>' + pegawai + '/' + bulan + '/' + tahun,
            type: 'POST',
            beforeSend: function() {
    ...
    $(document).on("click", "#tombol-cetak", function() {
        ...
        var bulan = $("#bulan").val();
        var tahun = $("#tahun").val();
        window.open ("<?= site_url('cetak-kehadiran-detail/'); ?>" + pegawai + "/" + tahun + "/" + bulan,"_blank");
    });
</script>
