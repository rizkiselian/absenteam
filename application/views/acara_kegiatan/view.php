...
<div class="panel-header bg-primary-gradient">
    <div class="page-inner py-4">
        <div class="d-flex align-items-left flex-column flex-sm-row">
            <h3 class="text-white pb-3 fw-bold">DATA ACARA DAN KEGIATAN</h3>
            <?php if ($role == "admin") : ?>
                            ...
                            <table id="tabel-data" class="table-default" style="width: 100%;">
                                <thead>
                                    <tr style="background-color: #1572EB; color: white;">
                                        ...
                                        <th colspan="2">ACUAN KOORDINAT</th>
                                        <?php if ($role == "admin") : ?>
                                            <th rowspan="2" style="width: 75px;">AKSI</th>
                                        <?php endif; ?>
                                    </tr>
                                    <tr style="background-color: #1572EB; color: white;">
                                        <th>JAM</th>
...
<script>
    ...
    <?php if ($role == "admin") : ?>
        ...
        function load_data() {
            ...
            table = $('#tabel-data').DataTable({
                ...
                ajax: {
                    url: '<?= site_url('load-acara-kegiatan'); ?>',
                    type: 'POST'
                },
                columnDefs: [{
    ...
    $(document).on("click", "#tombol-tambah", function() {
        ...
        $.ajax({
            url: '<?= site_url('form-add-acara-kegiatan'); ?>',
            type: "POST",
    ...
    $(document).on("click", "#btn-simpan", function() {
        ...
        $(this).html("<i class='fa fa-circle-notch fa-spin fa-sm'></i> LOADING...");
        $.ajax({
            url: "<?= site_url('add-acara-kegiatan'); ?>",
            type: 'POST',
    ...
    $(document).on("click", "#tombol-ubah", function() {
        $.ajax({
            url: '<?= site_url('form-edit-acara-kegiatan'); ?>',
            type: "POST",
            data: {
                id: $(this).data('id')
            },
    ...
    $(document).on("click", "#btn-ubah", function() {
        ...
        $(this).html("<i class='fa fa-circle-notch fa-spin fa-sm'></i> LOADING...");
        $.ajax({
            url: "<?= site_url('edit-acara-kegiatan'); ?>",
            type: 'POST',
    ...
    $(document).on("click", "#tombol-hapus", function(e) {
        ...
        swal({
            ...
            if (Delete) {
                $.ajax({
                    url: '<?= site_url('delete-acara-kegiatan'); ?>',
                    type: "POST",
                    data: {
                        id: $(this).data('id')
                    },
