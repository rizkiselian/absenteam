...
<div class="panel-header bg-primary-gradient">
    <div class="page-inner py-4">
        <div class="d-flex align-items-left flex-column flex-sm-row">
            <h3 class="text-white pb-3 fw-bold">DATA SHIFT PEGAWAI</h3>
                <div class="ml-sm-auto py-md-0">
                    <button id="tombol-tambah" class="btn btn-secondary btn-round btn-sm mr-2 mb-3" data-toggle="modal" data-target="#modal-tambah"><i class='fa fa-plus'></i> TAMBAH DATA</button>
                </div>
...
<script>
    $(document).ready(function() {
        ...
        function load_data(skpd) {
            ...
            table = $('#tabel-data').DataTable({
                ...
                ajax: {
                    url: '<?= site_url('load-data-shift-pegawai'); ?>',
                    type: 'POST',
                    data: {
                        skpd: skpd
                    }
    ...
    $(document).on("click", "#tombol-tambah", function() {
        ...
        $(this).html("<i class='fa fa-circle-notch fa-spin fa-sm'></i> LOADING...");
        $.ajax({
            url: '<?= site_url('form-add-data-shift-pegawai'); ?>',
            type: "POST",
            data: {
                skpd: "<?= $skpd; ?>"
            },
    ...
    $(document).on("click", "#btn-simpan", function() {
        ...
        $(this).html("<i class='fa fa-circle-notch fa-spin fa-sm'></i> LOADING...");
        $.ajax({
            url: "<?= site_url('add-data-shift-pegawai'); ?>",
            type: 'POST',
    ...
    $(document).on("click", "#tombol-ubah", function() {
        $.ajax({
            url: '<?= site_url('form-edit-data-shift-pegawai'); ?>',
            type: "POST",
            data: {
                id: $(this).data('id')
            },
    ...
    $(document).on("click", "#btn-ubah", function() {
        ...
        $(this).html("<i class='fa fa-circle-notch fa-spin fa-sm'></i> LOADING...");
        $.ajax({
            url: "<?= site_url('edit-data-shift-pegawai'); ?>",
            type: 'POST',
    ...
    $(document).on("click", "#tombol-hapus", function(e) {
        ...
        swal({
            ...
            if (Delete) {
                $.ajax({
                    url: '<?= site_url('delete-data-shift-pegawai'); ?>',
                    type: "POST",
                    data: {
                        id: $(this).data('id')
                    },
