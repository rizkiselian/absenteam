...
<div class="panel-header bg-primary-gradient">
    <div class="page-inner py-4">
        <div class="d-flex align-items-left align-items-md-center flex-column flex-md-row">
            <h3 class="text-white pb-3 fw-bold">TPP KEHADIRAN PEGAWAI</h3>
        </div>
    </div>
</div>
...
<?php if ($role == "admin") : ?>
    <script>
        ...
        function load_data(skpd, bulan, tahun) {
            $.ajax({
                url: '<?= site_url('load-tpp-kehadiran/'); ?>' + skpd + '/' + bulan + '/' + tahun,
                type: 'POST',
                beforeSend: function() {
        ...
        $(document).on("click", "#tombol-cetak", function() {
            ...
            var bulan = $("#bulan").val();
            var tahun = $("#tahun").val();
            window.open ("<?= site_url('cetak-kehadiran-tpp/'); ?>" + skpd + "/" + tahun + "/" + bulan,"_blank");
        });
    </script>
<?php else : ?>
    <script>
