...
<script>
    $(document).ready(function() {
        $('#form-ubah .select2').select2();
    });

    $("#form-ubah #unit_kerja").change(function() {
        var unit = $("#form-ubah #unit_kerja").val();
        $.ajax({
            url: '<?= site_url('load-pegawai-by-skpd'); ?>',
            type: "POST",
            data: {
                unit_kerja: $("#form-ubah #unit_kerja").val(),
                pegawai: "<?= encrypt_url($user['id_pegawai']); ?>"
            },
            success: function(data) {
                $('#form-ubah #nama_pegawai').html(data);
            }
        });
    });
