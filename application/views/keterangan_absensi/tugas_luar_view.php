...
<div class="panel-header bg-primary-gradient">
    <div class="page-inner py-4">
        <div class="d-flex align-items-left flex-column flex-sm-row">
            <h3 class="text-white pb-3 fw-bold">DATA PEGAWAI TUGAS LUAR</h3>
    ...
    <script>
        ...
        function load_data(skpd) {
            ...
            table = $('#tabel-data').DataTable({
                ...
                ajax: {
                    url: '<?= site_url('load-status-absensi/'); ?>' + skpd + '/tl',
                    type: 'POST'
                },
                columns: [{
                    data: 'no'
        ...
        $(document).on("click", "#tombol-tambah", function() {
            ...
            $(this).html("<i class='fa fa-circle-notch fa-spin fa-sm'></i> LOADING...");
            $.ajax({
                url: '<?= site_url('form-add-pegawai-tugas-luar'); ?>',
                type: "POST",
                data: {
                    unit_kerja: $("#skpd").val()
                },
        ...
        $(document).on("click", "#btn-simpan", function() {
            ...
            $(this).html("<i class='fa fa-circle-notch fa-spin fa-sm'></i> LOADING...");
            $.ajax({
                url: "<?= site_url('add-pegawai-tugas-luar'); ?>",
                type: 'POST',
        ...
        $(document).on("click", "#tombol-hapus", function(e) {
            ...
            swal({
                ...
                if (Delete) {
                    $.ajax({
                        url: '<?= site_url('delete-pegawai-tugas-luar'); ?>',
                        type: "POST",
                        data: {
                            keterangan_absensi: $(this).data('id')
                        },
                        success: function(data) {
                            reload_ajax();
                            if (data.notif) {
        ...
        function file_pdf(fileupload) {
            ...
            if (!extention_ok.exec(path_file)) {
                ...
                if (input_file.files && input_file.files[0]) {
                    if (input_file.files[0].size > 768000) {
                        swal({
                            icon: 'error',
                            title: 'KONFIRMASI',
                            text: 'UKURAN FILE HARUS DI BAWAH ' + (768000 / 1024) + ' KB',
                            timer: 1500
                        })
                        input_file.value = '';
                        return false;
